...
if (logoutButton) {
    logoutButton.addEventListener('click', function (e) {
        e.preventDefault(); // Ngăn trình duyệt reload hoặc nhảy link mặc định
        
        // Xóa trạng thái đăng nhập của Admin trong bộ nhớ trình duyệt (nếu có)
        localStorage.removeItem('adminLoggedIn'); 
        
        // Điều hướng quay trở lại trang chủ index.html của Tung Food
...
// 1. Hàm lấy dữ liệu an toàn (Tự động chuyển về Local nếu lỗi Database)
async function getOrdersFromDatabase() {
    try {
        const response = await fetch('http://localhost:3000/api/orders');
        if (!response.ok) {
            throw new Error("Không kết nối được database");
        }
        return await response.json();
    } catch (error) {
        console.warn("⚠️ Đang dùng dữ liệu đơn hàng Local do lỗi database:", error.message);
        let localOrders = JSON.parse(localStorage.getItem("order")) || [];
        return localOrders;
    }
}

// 2. Hàm tính tổng tiền và hiển thị Dashboard đồng bộ hệ thống
async function updateDashboard() {
    try {
        const orders = await getOrdersFromDatabase(); // Đợi dữ liệu tải về từ backend/local
        let tongTien = 0;
        
        orders.forEach(item => {
            // Tính tổng tiền đơn hàng (hỗ trợ cả trường tongtien local hoặc totalPrice từ Atlas)
            tongTien += Number(item.tongtien || item.totalPrice || 0);
        });
        ...
        // Đổ dữ liệu an toàn lên giao diện Admin
        const doanhThuElement = document.getElementById('doanh-thu');
        if (doanhThuElement) {
            doanhThuElement.innerHTML = VND.format(tongTien);
        }

        const soLuongDonElement = document.getElementById('so-luong-don');
        if (soLuongDonElement) {
            soLuongDonElement.innerHTML = orders.length;
        }

        const soKhachHangElement = document.getElementById('so-luong-khach');
        if (soKhachHangElement) {
            soKhachHangElement.innerHTML = getAmoumtUser(); 
        }

        // Cập nhật hiển thị số lượng thống kê phụ bên dưới
        const amountUserEl = document.getElementById("amount-user");
        if (amountUserEl) amountUserEl.innerHTML = getAmoumtUser();

        const amountProductEl = document.getElementById("amount-product");
        if (amountProductEl) amountProductEl.innerHTML = getAmoumtProduct();
...
// Kích hoạt hàm chạy duy nhất để load toàn bộ số liệu lên màn hình chính
updateDashboard();
